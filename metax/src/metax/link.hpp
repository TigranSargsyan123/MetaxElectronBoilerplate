...
#include <platform/sha256_generator.hpp>
...
private:
        ...
        void send_public_key_notification_to_peer(const std::string& m,
                        const std::string& pk, const std::string& hash,
                        const std::string& ip);
...
private:
        ...
        private:
                void send_user_keys();
                void send_user_public_key();
                void send_channel_to_pair();
                std::string name()
                {
        ...
        void save_start_channel_pairing_request_info();
        ...
        void send_channel_fail_message(const std::string& m, metax::command cmd);
        void send_generated_code();
        void send_channel_generated_code();
        void on_expired_start_pairing(Poco::Timer&);
        void handle_config_start_pairing();
        void handle_config_start_channel_pairing();
        void cancel_pairing();
        ...
        std::string m_open_channel_uuid;
        std::string m_open_channel_aes_key;
        std::string m_open_channel_aes_iv;
        Poco::Mutex m_notification_mutex;
        std::string m_notification;
        std::string m_notification_public_key;
        std::string m_notification_hash;
        std::string m_notification_ip;
...
private:
        ...
        void get_user_public_key(Poco::Net::SecureStreamSocket& client, ID32 req_id);
        void get_channel_of_pair(Poco::Net::SecureStreamSocket& client,
                        ID32 req_id, const std::string& own_uuid);
        void connect_server_and_save_keys(const std::string& current_address,
                const std::string& code, ID32 req_id);
        void connect_server_and_get_public_key(const std::string& current_address,
                const std::string& code, ID32 req_id);
        void connect_server_and_open_channel(const std::string& current_address,
                ID32 req_id, const Poco::UUID& uuid, const std::string& aes_key,
                const std::string& aes_iv);
        void handle_config_request_keys();
        void handle_config_open_channel();
        void handle_config_pair_channel_accept_share();
        void handle_config_request_public_key();
