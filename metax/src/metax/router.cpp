...
configure_router()
{
        ...
        CACHING_ROUTER = in.caching_router;
...
bypass_tender_response(const router_link_package* winner)
{
        ...
        if (2 == CACHING_ROUTER) {
                best.extract_payload(*winner, best.header_size());
                kernel_storage_package& storage_out = *storage_writer_tx;
                storage_out.request_id = winner->request_id;
                storage_out.cmd = metax::save_data;
                storage_out.uuid = best.uuid;
                storage_out.set_payload(best);
                storage_writer_tx.commit();
        }
        else if (1 == CACHING_ROUTER) {
                best.extract_payload(*winner, best.header_size());
                kernel_storage_package& cache_out = *cache_tx;
                cache_out.request_id = winner->request_id;
                cache_out.cmd = metax::add_in_cache;
                cache_out.uuid = best.uuid;
                cache_out.set_payload(best);
                cache_tx.commit();
        }
        link_tx.commit();
        t.time.update();
