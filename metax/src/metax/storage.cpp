...
save_data()
{
        ...
        if (in.should_send_response) {
                Poco::ScopedLock<Poco::Mutex> m(m_kernel_mutex);
                kernel_storage_package& out = *kernel_tx;
                out.uuid = in.uuid;
                out.request_id = in.request_id;
                out.cmd = cmd;
...
save_router_data()
{
        ...
        const kernel_storage_package& in = *router_rx;
        poco_assert(metax::save_data == in.cmd);
        poco_assert(nullptr != s_db);
        std::string msg;
        try {
                ...
                platform::default_package data, uint32_t data_version)
{
        ...
        try {
                ...
                router_tx.commit();
                notify_router_update_to_kernel(id, uuid, data_version);
        } catch (const Poco::Exception& e) {
...
void leviathan::metax::storage::
notify_router_update_to_kernel(ID32 id,Poco::UUID uuid,
                                        uint32_t data_version)
{
        ...
        out.uuid = uuid;
        out.request_id = id;
        out.cmd = metax::router_update_confirm;
...
process_router_input()
{
        ...
        switch (in.cmd) {
                case metax::save_data: {
                        ...
                        METAX_WARNING("Unhandled command from router");
                }
        }
        METAX_TRACE(std::string("END ") + __FUNCTION__);
}
