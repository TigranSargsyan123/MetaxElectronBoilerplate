...
void leviathan::metax::configuration_manager::
handle_kernel_pair_channel_accept_share()
{
        METAX_TRACE(__FUNCTION__);
        poco_assert(kernel_rx.has_data());
        const metax::kernel_cfg_package& in = *kernel_rx;
...
void leviathan::metax::configuration_manager::
handle_kernel_start_channel_pairing()
{
        METAX_TRACE(__FUNCTION__);
        poco_assert(kernel_rx.has_data());
        const metax::kernel_cfg_package& in = *kernel_rx;
...
void leviathan::metax::configuration_manager::
handle_kernel_request_public_key()
{
        METAX_TRACE(__FUNCTION__);
        poco_assert(kernel_rx.has_data());
        const metax::kernel_cfg_package& in = *kernel_rx;
...
void leviathan::metax::configuration_manager::
handle_link_pair_channel_response()
{
        METAX_TRACE(__FUNCTION__);
        poco_assert(link_rx.has_data());
        const metax::link_cfg_package& in = *link_rx;
...
void leviathan::metax::configuration_manager::
handle_link_pair_channel_accept()
{
        METAX_TRACE(__FUNCTION__);
        poco_assert(link_rx.has_data());
        const metax::link_cfg_package& in = *link_rx;
...
void leviathan::metax::configuration_manager::
handle_link_get_channel_generated_code()
{
        METAX_TRACE(__FUNCTION__);
        poco_assert(link_rx.has_data());
        const metax::link_cfg_package& in = *link_rx;
...
void leviathan::metax::configuration_manager::
handle_link_start_channel_pairing_fail()
{
        METAX_TRACE(__FUNCTION__);
        poco_assert(link_rx.has_data());
        const metax::link_cfg_package& in = *link_rx;
        ...
        kernel_cfg_package& out = *kernel_tx;
        out.request_id = in.request_id;
        out.cmd = metax::start_channel_pairing_fail;
        out = in;
        kernel_tx.commit();
        METAX_TRACE(std::string("END ") + __FUNCTION__);
}
...
initialize_router()
{
        ...
        (*router_tx).caching_router = cfg_data.caching_router;
...
void leviathan::metax::configuration_manager::
get_caching_router(const Poco::Util::XMLConfiguration* c)
{
        METAX_TRACE(__FUNCTION__);
        poco_assert(nullptr != c);
        std::string m = "";
        try {
                cfg_data.caching_router = c->getUInt("router.caching_router");
                if (cfg_data.caching_router < 3) {
                        METAX_TRACE(std::string("END ") + __FUNCTION__);
                        return;
                }

                m = "Invalid caching_router value, should be in [0 - 2]";
        } catch(Poco::RuntimeException& e) {
                m = e.displayText();
        }
        METAX_WARNING(m + ", defaulting caching_router to 0");
        cfg_data.caching_router = 0;
        METAX_TRACE(std::string("END ") + __FUNCTION__);
}
