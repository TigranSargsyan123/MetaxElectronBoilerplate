...
public:
        ...
        using ROUTER_RX = platform::csp::input<kernel_storage_package>;
...
public:
         ...
         * @param data - payload to be saved
         * @param data_version - data version of payload
         */
        void update_data_async_for_router(ID32 id, Poco::UUID uuid,
                        platform::default_package data, uint32_t data_version);

        /**
         * @brief Sends router's update response to kernel for notifying. This
         * function is run from a separate thread, so before pushing data into
         * kernel, the kernel tx is locked by mutex
         *
         * @param id - request id
         * @param uuid - uuid to be updated
         * @param data_version - data version of payload
         */
        void notify_router_update_to_kernel(ID32 id,Poco::UUID uuid,
