...
void leviathan::metax::link::
save_start_channel_pairing_request_info()
{
        METAX_TRACE(__FUNCTION__);
        poco_assert(config_rx.has_data());
        const link_cfg_package& in = *config_rx;
        poco_assert(metax::start_channel_pairing == in.cmd);
        poco_assert(nullptr != in.message.get());
...
void leviathan::metax::link::
send_channel_fail_message(const std::string& m, metax::command cmd)
{
        METAX_TRACE(__FUNCTION__);
        poco_assert(config_rx.has_data());
        const link_cfg_package& in = *config_rx;
        ...
        Poco::JSON::Object::Ptr p = new Poco::JSON::Object();
        std::string fail =
                metax::start_channel_pairing_fail == cmd ? "error" : "warning";
        p->set(fail, m);
        std::stringstream ss;
        p->stringify(ss);
        link_cfg_package& out = *config_tx;
...
send_channel_generated_code()
{
        ...
        poco_assert(metax::start_channel_pairing == in.cmd);
        link_cfg_package& out = *config_tx;
        out.cmd = metax::get_channel_generated_code;
        m_open_channel_uuid = in.uuid.toString();
        m_open_channel_aes_key = in.aes_key;
        m_open_channel_aes_iv = in.aes_iv;
        out.request_id = in.request_id;
        out.set_payload(ss.str());
        config_tx.commit();
        METAX_TRACE(std::string("END ") + __FUNCTION__);
...
on_expired_start_pairing(Poco::Timer&)
{
        ...
        if (nullptr != m_pairing_socket) {
                ...
                m_generated_code = "";
                m_open_channel_uuid = "";
                m_open_channel_aes_key = "";
                m_open_channel_aes_iv = "";
        }
        METAX_TRACE(std::string("END ") + __FUNCTION__);
}

void leviathan::metax::link::
handle_config_pair_channel_accept_share()
{
        METAX_TRACE(__FUNCTION__);
        poco_assert(config_rx.has_data());
        const link_cfg_package& in = *config_rx;
        poco_assert(metax::pair_channel_accept_share == in.cmd);
        Poco::JSON::Object::Ptr p = new Poco::JSON::Object();
        poco_assert(nullptr != p);
        p->set("open_channel", "received open_channel rx-tx pair");
        p->set("rx", in.uuid.toString());
        p->set("tx", in.message.get());
        std::stringstream ss;
        p->stringify(ss);
        link_cfg_package& out = *config_tx;
...
void leviathan::metax::link::
handle_config_start_channel_pairing()
{
        METAX_TRACE(__FUNCTION__);
        if (nullptr != m_pairing_socket) {
                send_channel_fail_message("Server already started",
                                metax::start_channel_pairing_fail);
                return;
        }
        try {
                save_start_channel_pairing_request_info();
                ...
                METAX_INFO(std::string("Open channel: failed to start server ")
                                                + std::string(e.displayText()));
                send_channel_fail_message("Open channel: failed to start server "
                                + e.displayText(), metax::start_channel_pairing_fail);
        } catch (...) {
                send_channel_fail_message("Open channel: failed to start server ",
                                                metax::start_channel_pairing_fail);
        }
...
void leviathan::metax::link::
get_channel_of_pair(Poco::Net::SecureStreamSocket& client, ID32 req_id,
                const std::string& own_uuid)
{
        platform::default_package pkg = link_peer::read_message(client);
        key_transfer_package ktp;
        ktp.extract_header(pkg);
        std::string aes_key = ktp.public_key;
        link_cfg_package& out = *config_tx;
        out.aes_key = ktp.public_key;
        out.aes_iv = ktp.private_key;
        Poco::UUID uuid(ktp.user_json_info);
        out.uuid = uuid;
        out.set_payload(own_uuid);
        out.request_id = req_id;
        out.cmd = metax::request_pair_channel_accept;
        config_tx.commit();
}

void leviathan::metax::link::
get_user_public_key(Poco::Net::SecureStreamSocket& client, ID32 req_id)
{
        platform::default_package pkg = link_peer::read_message(client);
        key_transfer_package ktp;
        ktp.extract_header(pkg);
        ...
        out.request_id = req_id;
        out.cmd = metax::request_public_key_response;
                ...
                const std::string& code, ID32 req_id)
{
        ...
        if ("Connected" == std::string(hm.message.get(), hm.size)) {
                key_transfer_package ktp;
                ktp.code = code;
                ktp.request_type = "request_key_pair";
...
void leviathan::metax::link::
handle_config_open_channel()
{
        METAX_TRACE(__FUNCTION__);
        poco_assert(config_rx.has_data());
        const link_cfg_package& in = *config_rx;
        poco_assert(metax::open_channel == in.cmd);
        std::string c = in.ip + ":" + platform::utils::to_string(m_pairing_port);
        try {
                connect_server_and_open_channel(c, in.request_id, in.uuid,
                                in.aes_key, in.aes_iv);
...
void leviathan::metax::link::
handle_config_request_public_key()
{
        METAX_TRACE(__FUNCTION__);
        poco_assert(config_rx.has_data());
        const link_cfg_package& in = *config_rx;
        poco_assert(metax::request_public_key == in.cmd);
        ...
        try {
                connect_server_and_get_public_key(c, code, in.request_id);
        } catch (const Poco::Net::NetException& e) {
                std::string m = "Transfer keys: Fail to connect " +
                        c + std::string(e.displayText());
                send_request_keys_fail_to_config(m, in.request_id);
...
void leviathan::metax::link::
connect_server_and_open_channel(const std::string& current_address,
                ID32 req_id, const Poco::UUID& uuid,
                const std::string& aes_key, const std::string& aes_iv)
{
        METAX_TRACE(__FUNCTION__);
        const Poco::Net::SocketAddress sa(current_address);
        Poco::Net::SecureStreamSocket client;
        ...
        if ("Connected" == std::string(hm.message.get(), hm.size)) {
                key_transfer_package ktp;
                ktp.request_type = "open_channel";
                ktp.public_key = aes_key;
                ktp.private_key = aes_iv;
                ktp.user_json_info = uuid.toString();
                platform::default_package cm;
                ktp.serialize(cm);
                link_peer::send_by_socket(client, cm);
                get_channel_of_pair(client, req_id, uuid.toString());
        }
        METAX_TRACE(std::string("END ") + __FUNCTION__);
}

void leviathan::metax::link::
connect_server_and_get_public_key(const std::string& current_address,
                const std::string& code, ID32 req_id)
{
        METAX_TRACE(__FUNCTION__);
        const Poco::Net::SocketAddress sa(current_address);
        ...
        if ("Connected" == std::string(hm.message.get(), hm.size)) {
                ...
                ktp.public_key = platform::utils::read_file_content(
                                this->m_user_key_path + "/public.pem", false);
                poco_assert(! ktp.public_key.empty());
                ktp.request_type = "request_public_key";
                platform::default_package cm;
                ktp.serialize(cm);
                link_peer::send_by_socket(client, cm);
                get_user_public_key(client, req_id);
        }
        METAX_TRACE(std::string("END ") + __FUNCTION__);
}
...
process_config_input()
{
        ...
        switch ((*config_rx).cmd) {
                case metax::device_id: {
                        ...
                        handle_config_start_pairing();
                        break;
                } case metax::pair_channel_accept_share: {
                        handle_config_pair_channel_accept_share();
                        break;
                } case metax::start_channel_pairing: {
                        handle_config_start_channel_pairing();
                        break;
                        ...
                        handle_config_get_pairing_peers();
                        break;
                } case metax::open_channel: {
                        handle_config_open_channel();
                        break;
                } case metax::request_public_key: {
                        handle_config_request_public_key();
                        break;
...
void leviathan::metax::link::
send_public_key_notification_to_peer(const std::string& m, const std::string& pk,
                const std::string& hash, const std::string& ip)
{
        METAX_TRACE(__FUNCTION__);
        Poco::JSON::Object::Ptr p = new Poco::JSON::Object();
        poco_assert(nullptr != p);
...
handle_key_transfer_notification()
{
        ...
        if (! m_notification.empty()) {
               if(!m_notification_public_key.empty()) {
                        send_public_key_notification_to_peer(m_notification,
                                        m_notification_public_key,
                                        m_notification_hash,
                                        m_notification_ip); 
                        m_notification = "";
                        m_notification_public_key = "";
...
send_channel_to_pair()
{
        METAX_TRACE(__FUNCTION__);
        poco_assert(nullptr != m_link);
        key_transfer_package ktp;
        ktp.public_key = m_link->m_open_channel_aes_key;
        ktp.private_key = m_link->m_open_channel_aes_iv;
        ktp.user_json_info = m_link->m_open_channel_uuid;
        poco_assert(! ktp.public_key.empty());
...
try {
        ...
        key_transfer_package ktp;
        ktp.extract_header(pkg);
        if ("open_channel" == ktp.request_type) {
                send_channel_to_pair();
                link_cfg_package& out = *m_link->config_tx;
                out.aes_key = ktp.public_key;
                out.aes_iv = ktp.private_key;
                Poco::UUID uuid(ktp.user_json_info);
                out.uuid = uuid;
                out.set_payload(m_link->m_open_channel_uuid);
                out.cmd = metax::request_pair_channel_response;
                m_link->config_tx.commit();
                m_link->cancel_pairing();
                METAX_TRACE(std::string("END ") + __FUNCTION__);
                return;
        }
        ...
        if (ktp.code == m_link->m_generated_code) {
                if ("request_public_key" == ktp.request_type) {
                        send_user_public_key();
                        Poco::Mutex::ScopedLock lock(m_link->m_notification_mutex);
                        m_link->m_notification = "Successfully received public key of " + s + " and sent own public key";
                        m_link->m_notification_public_key = ktp.public_key;
                        std::string hash_key = ktp.public_key;
                        hash_key.erase(std::remove(hash_key.begin(), hash_key.end(),
                                                '\n'), hash_key.end());
                        leviathan::platform::sha256_generator sha;
                        sha.update(hash_key);
                        uint8_t * digest = sha.digest();
                        std::string hash = leviathan::platform::sha256_generator::toString(digest);
                        delete[] digest;
                        m_link->m_notification_hash = hash;
                        m_link->m_notification_ip = s;
                        m_link->wake_up();
                        m_link->cancel_pairing();
                } else if ("request_key_pair" == ktp.request_type) {
                        send_user_keys();
                        Poco::Mutex::ScopedLock lock(m_link->m_notification_mutex);
