...
process_link_input()
{
        ...
        switch (in.cmd) {
                case metax::request_public_key_response: {
                        handle_link_request_public_key_response(in);
                        break;
...
void leviathan::metax::key_manager::
handle_link_request_public_key_response(const link_key_manager_package& in)
try {
        poco_assert(in.cmd == metax::request_public_key_response);
        kernel_key_manager_package& out = (*kernel_tx); 
