...
handle_decrypted_key(request& r)
{
        ...
        if(r.cmd == metax::accept_channel_share) {
                ims_kernel_package& out = *ims_tx;
                out.cmd = metax::share_channel_accepted;
                out.request_id = m_ims_req_ids[r.request_id];
                out.set_payload(r.file_path);
                out.uuid = r.uuid;
                ims_tx.commit();
        ...
        if(r.cmd == metax::pair_channel_accept_share) {
                kernel_cfg_package& out = *config_tx;
                out.cmd = metax::pair_channel_accept_share;
                out.set_payload(r.file_path);
                out.uuid = r.uuid;
                config_tx.commit();
                m_requests.erase(r.request_id);
...
handle_key_manager_decrypted_request()
{
        ...
        switch (r.cmd) {
                case metax::get: {
                  ...
                  case metax::accept_channel_share: {
...
process_key_manager_input()
{
        ...
        if (m_requests.end() == m_requests.find(in.request_id)
                        && (metax::request_keys_response != in.cmd
                                ...
                                && metax::request_public_key_response != in.cmd
                                && metax::request_public_key_fail != in.cmd
        ...
        switch (in.cmd) {
                case metax::encrypted: {
                        ...
                        handle_user_keys_regeneration_fail();
                        break;
                } case metax::request_public_key_response: {
                        handle_key_manager_request_public_key_response(in);
                        break;
                } case metax::request_keys_response: {
                        handle_key_manager_request_keys_response(in);
                        break;
                } case metax::request_public_key_fail: {
                        handle_request_public_key_fail(in.request_id, in);
                        break;
...
process_storage_writer_input()
{
        ...
        switch (in.cmd) {
                case metax::save_confirm: {
                        ...
                        break;
                } case metax::router_update_confirm: {
                        handle_storage_router_update_confirm();
                        break;
                } default: {
...
handle_router_update_request()
{
        ...
        if (m_save_storage->look_up_data_in_db(r.uuid, false)) {
                ...
                std::thread t(&storage::update_data_async_for_router,
                        m_save_storage, in.request_id, in.uuid, in, r.data_version);
                t.detach();
        }
        METAX_TRACE(std::string("END ") + __FUNCTION__);
}
...
handle_storage_router_update_confirm()
{
        ...
        poco_assert(storage_writer_rx.has_data());
        const kernel_storage_package& in = *storage_writer_rx;
        poco_assert(metax::router_update_confirm == in.cmd);
        notify_update(in.uuid, metax::update, in.data_version);
        METAX_TRACE(std::string("END ") + __FUNCTION__);
}
        ...
        switch (in.cmd) {
                case metax::kernel_cfg: {
                  ...
                  case metax::add_friend_confirm: {
                        ...
                        handle_config_get_public_key_response();
                        break;
                } case metax::request_pair_channel_response: {
                        handle_config_request_pair_channel_response();
                        break;
                } case metax::request_pair_channel_accept: {
                        handle_config_request_pair_channel_accept();
                        break;
                        ...
                        break;
                } case metax::get_channel_generated_code: {
                        handle_config_get_channel_generated_code();
                        break;
                } case metax::start_pairing_fail:
                  case metax::get_generated_code: {
                        handle_config_get_generated_code();
                        break;
                } case metax::start_channel_pairing_fail: {
                        handle_config_start_channel_pairing_fail();
                        break;
                } case metax::request_public_key_fail: {
                        handle_request_public_key_fail(in.request_id, in);
                        break;
...
void leviathan::metax::kernel::
handle_ims_pair_channel_accept_share()
{
        METAX_TRACE(__FUNCTION__);
        poco_assert(ims_rx.has_data());
        const ims_kernel_package& in = *ims_rx;
        ...
        poco_assert(m_requests.end() == m_requests.find(id));
        request& r = m_requests[id];
        const std::string& own_uuid = in.message.get();
        r = request(id, in.uuid, request::requested, metax::pair_channel_accept_share,
                        request::t_ims, own_uuid);
        kernel_key_manager_package& out = *key_manager_tx;
...
void leviathan::metax::kernel::
handle_ims_accept_channel_share_request()
{
        METAX_TRACE(__FUNCTION__);
        poco_assert(ims_rx.has_data());
        const ims_kernel_package& in = *ims_rx;
        poco_assert(metax::accept_channel_share == in.cmd);
        ims_kernel_package& out = *ims_tx;
        if (in.uuid.isNull()) {
                out.cmd = metax::accept_share_fail;
                out.request_id = in.request_id;
                ...
                process_ims_accept_channel_share_request();
        }
        METAX_TRACE(std::string("END ") + __FUNCTION__);
}
...
void leviathan::metax::kernel::
handle_ims_start_channel_pairing()
{
        METAX_TRACE(__FUNCTION__);
        poco_assert(ims_rx.has_data());
        const ims_kernel_package& in = *ims_rx;
...
void leviathan::metax::kernel::
handle_ims_request_user_public_key()
{
        METAX_TRACE(__FUNCTION__);
        poco_assert(ims_rx.has_data());
        const ims_kernel_package& in = *ims_rx;
...
process_ims_accept_channel_share_request()
{
        ...
        request& r = m_requests[id];
        const std::string& own_uuid = in.message.get();
        r = request(id, in.uuid, request::requested, metax::accept_channel_share,
                        request::t_ims, own_uuid);
        poco_assert(m_ims_req_ids.end() == m_ims_req_ids.find(id));
        m_ims_req_ids[id] = in.request_id;
        kernel_key_manager_package& out = *key_manager_tx;
...
void leviathan::metax::kernel::
handle_ims_open_channel_request()
{
        METAX_TRACE(__FUNCTION__);
        poco_assert(ims_rx.has_data());
        const ims_kernel_package& in = *ims_rx;
...
handle_config_start_channel_pairing_fail()
{
        ...
        poco_assert(config_rx.has_data());
        const kernel_cfg_package& in = *config_rx;
        poco_assert(metax::start_channel_pairing_fail == in.cmd);
        ims_kernel_package& out = *ims_tx;
        poco_assert(m_ims_req_ids.end() != m_ims_req_ids.find(in.request_id));
        out.request_id = m_ims_req_ids[in.request_id];
        out = in;
...
handle_config_get_channel_generated_code()
{
        ...
        poco_assert(config_rx.has_data());
        const kernel_cfg_package& in = *config_rx;
        poco_assert(metax::get_channel_generated_code == in.cmd ||
                        metax::start_channel_pairing_fail == in.cmd);
        ims_kernel_package& out = *ims_tx;
        poco_assert(m_ims_req_ids.end() != m_ims_req_ids.find(in.request_id));
        out.request_id = m_ims_req_ids[in.request_id];
        out = in;
...
void leviathan::metax::kernel::
handle_key_manager_request_public_key_response(const kernel_key_manager_package& in)
{
        METAX_TRACE(__FUNCTION__);
        poco_assert(metax::request_public_key_response == in.cmd);
        ims_kernel_package& out = *ims_tx;
        poco_assert(m_ims_req_ids.end() != m_ims_req_ids.find(in.request_id));
        out.request_id = m_ims_req_ids[in.request_id];
        out = in;
...
handle_config_request_pair_channel_response()
{
        METAX_TRACE(__FUNCTION__);
        const kernel_cfg_package& in = *config_rx;
        poco_assert(metax::request_pair_channel_response == in.cmd);
        ims_kernel_package& out = *ims_tx;
        out.aes_key = in.aes_key;
        out.aes_iv = in.aes_iv;
        out.uuid = in.uuid;
...
handle_config_request_pair_channel_accept()
{
        METAX_TRACE(__FUNCTION__);
        const kernel_cfg_package& in = *config_rx;
        poco_assert(metax::request_pair_channel_accept == in.cmd);
        ID32 id = in.request_id;
        ims_kernel_package& out = *ims_tx;
        out.request_id = m_ims_req_ids[id];
        out.aes_key = in.aes_key;
...
void leviathan::metax::kernel::
handle_request_public_key_fail(ID32 request_id, platform::default_package in)
{
        METAX_TRACE(__FUNCTION__);
        ims_kernel_package& out = *ims_tx;
        ...
        out = in;
        out.cmd = metax::request_public_key_fail;
        ims_tx.commit();
