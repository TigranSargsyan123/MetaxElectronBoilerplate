...
void leviathan::metax_web_api::web_api_adapter::
handle_open_channel_request(std::promise<ims_response>* pr, const std::string& s,
                const std::string& ip)
{
        METAX_TRACE(__FUNCTION__);
        METAX_NOTICE("Processing open_channel request for " + ip + " ip");
        const metax::ID32 id = generate_request_id();
        {
                Poco::ScopedLock<Poco::Mutex> sm(m_save_requests_mutex);
                m_save_requests.insert(std::make_pair(id, request_info(id, pr,
                                "", s, Poco::UUID::null(), metax::open_channel, ip)));
        }
        try {
                Poco::ScopedLock<Poco::Mutex> gm(m_metax_tx_mutex);
                metax::ims_kernel_package& out = *metax_tx;
                out.request_id = id;
                out.cmd = metax::save;
                out.uuid = Poco::UUID::null();
                out.local = metax::ims_kernel_package::DEFAULT;
                out.enc = true;
                out.set_payload(std::string("{}"));
                out.expire_date = DEFAULT_EXPIRE_DATE;
...
void leviathan::metax_web_api::web_api_adapter::
handle_request_pair_channel_response() {
        METAX_TRACE(__FUNCTION__);
        const metax::ID32 id = generate_request_id();
        Poco::ScopedLock<Poco::Mutex> gm(m_metax_tx_mutex);
...
void leviathan::metax_web_api::web_api_adapter::
handle_request_pair_channel_accept() {
        METAX_TRACE(__FUNCTION__);
        ...
        out.cmd = metax::accept_channel_share;
        out.uuid = in.uuid;
        const std::string& own_uuid = in.message.get();
        out.set_payload(own_uuid);
        out.aes_key = in.aes_key;
...
void leviathan::metax_web_api::web_api_adapter::
handle_start_channel_pairing(std::promise<web_api_adapter::ims_response>* pr,
                        const std::string& timeout, const std::string& user)
{
        METAX_TRACE(__FUNCTION__);
        poco_assert(nullptr != pr);
        const metax::ID32 id = generate_request_id();
        {
                Poco::ScopedLock<Poco::Mutex> sm(m_config_requests_mutex);
                m_save_requests.insert(std::make_pair(id, request_info(id, pr,
                                timeout, user, Poco::UUID::null(),
                                metax::start_channel_pairing, "")));
        }
        try {
                Poco::ScopedLock<Poco::Mutex> gm(m_metax_tx_mutex);
                metax::ims_kernel_package& out = *metax_tx;
...
void leviathan::metax_web_api::web_api_adapter::
handle_request_public_key(std::promise<web_api_adapter::ims_response>* pr,
                        const std::string& ip, const std::string& code)
{
        METAX_TRACE(__FUNCTION__);
        poco_assert(nullptr != pr);
        ...
        out.set_payload(payload);
        out.request_id = id;
        out.cmd = metax::request_public_key;
        metax_tx.commit();
        METAX_TRACE(std::string("END ") + __FUNCTION__);
}
...
handle_get_user_public_key_response()
{
        ...
        if (m_config_requests.end() != i) {
                ...
                std::string hash_key = in.message.get();
...
void leviathan::metax_web_api::web_api_adapter::
handle_get_channel_generated_code()
{
        METAX_TRACE(__FUNCTION__);
        const metax::ims_kernel_package& in = *metax_rx;
        ...
        if (m_save_requests.end() != i) {
                ...
                poco_assert(nullptr != pr);
                ims_response ir(content, Poco::Net::HTTPResponse::HTTP_OK,
                                R"({"channel_socket":"opened"})", "application/json");
                pr->set_value(ir);
                clean_save_request(in.request_id);
        }
        METAX_TRACE(std::string("END ") + __FUNCTION__);
...
void leviathan::metax_web_api::web_api_adapter::
handle_start_channel_pairing_fail()
{
        METAX_TRACE(__FUNCTION__);
        const metax::ims_kernel_package& in = *metax_rx;
        poco_assert(metax::start_channel_pairing_fail == in.cmd ||
                        metax::share_fail == in.cmd);
        std::map<metax::ID32, request_info>::iterator i;
        {
                Poco::ScopedLock<Poco::Mutex> sm(m_config_requests_mutex);
                i = m_save_requests.find(in.request_id);
        ...
        if (m_config_requests.end() != i) {
                ...
                poco_assert(nullptr != in.message.get());
                std::string m;
                if(metax::start_channel_pairing_fail == in.cmd) {
                        m = in.message.get();
                } else {
                        m = R"({"error":"Failed to share channel with given public key."})";
                }
                ims_response ir(content,
...
void leviathan::metax_web_api::web_api_adapter::
handle_request_public_key_response(const metax::ims_kernel_package& resp)
{
        METAX_TRACE(__FUNCTION__);
        const metax::ims_kernel_package& in = *metax_rx;
        ...
        if (m_config_requests.end() != i) {
                std::promise<ims_response>* pr = (i->second).prom;
                poco_assert(nullptr != pr);
                std::string hash_key = in.public_key;
                hash_key.erase(std::remove(hash_key.begin(), hash_key.end(),
                                        '\n'), hash_key.end());
                leviathan::platform::sha256_generator sha;
                sha.update(hash_key);
                ...
                Poco::JSON::Object::Ptr j = new Poco::JSON::Object();
                j->set("KeyTransfer", "received public key successfully");
                j->set("public_key",in.public_key);
                j->set("hash", hash);
                std::stringstream ss;
                j->stringify(ss);
                ims_response ir(content, Poco::Net::HTTPResponse::HTTP_OK,
...
send_save_response()
{
        ...
        if((i->second).command == metax::open_channel ||
                (i->second).command == metax::start_channel_pairing) {
                Poco::ScopedLock<Poco::Mutex> gm(m_metax_tx_mutex);
                metax::ims_kernel_package& out = *metax_tx;
                out.request_id = in.request_id;
                out.cmd = metax::share;
                out.uuid = in.uuid;
                out.set_payload((i->second).username);
                metax_tx.commit();
                METAX_TRACE(std::string("END ") + __FUNCTION__);
                return; 
        }
...
handle_share_confirm()
{
        ...
        if((i->second).command == metax::open_channel ||
                ...
                out.request_id = in.request_id;
                if((i->second).command == metax::open_channel) {
                        out.cmd = metax::open_channel;
                        out.set_payload((i->second).ip); 
                } else {
                        out.set_payload((i->second).info);
                        out.cmd = metax::start_channel_pairing;
                }
                out.uuid = in.uuid;
                out.aes_key = in.aes_key;
                out.aes_iv = in.aes_iv;
...
handle_open_channel_response()
{
        ...
        if (m_save_requests.end() != i) {
                std::promise<ims_response>* pr = (i->second).prom;
                poco_assert(nullptr != pr);
                poco_assert(metax::open_channel == (i->second).command);
                std::string s = R"({"rx":")" + in.uuid.toString() + 
                        R"(","tx":")" + in.message.get() + R"("})";
                ims_response ir(content, Poco::Net::HTTPResponse::HTTP_OK,
                                s, "application/json");
                pr->set_value(ir);
                clean_save_request(in.request_id);
        ...
        switch (in.cmd) {
                ...
                case metax::save_confirm : {
                  ...
                  case metax::update_fail: {
                        ...
                        handle_accept_share_fail();
                        break;
                } case metax::share_channel_accepted: {
                        handle_open_channel_response();
                        break;
                        ...
                        handle_request_public_key_response(in);
                        break;
