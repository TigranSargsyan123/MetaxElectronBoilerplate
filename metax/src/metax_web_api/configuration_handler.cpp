...
const std::string leviathan::metax_web_api::configuration_handler::
URI_CONFIG_START_CHANNEL_PAIRING = "/start_channel_pairing";
const std::string leviathan::metax_web_api::configuration_handler::
URI_CONFIG_CANCEL_PAIRING = "/cancel_pairing";
const std::string leviathan::metax_web_api::configuration_handler::
URI_CONFIG_GET_PAIRING_PEERS = "/get_pairing_peers";
const std::string leviathan::metax_web_api::configuration_handler::
URI_CONFIG_CONNECT_SERVER_PUBLIC = "/request_public_key";
const std::string leviathan::metax_web_api::configuration_handler::
URI_CONFIG_CONNECT_SERVER = "/request_keys";
const std::string leviathan::metax_web_api::configuration_handler::
URI_CONFIG_OPEN_CHANNEL = "/open_channel";
const std::string leviathan::metax_web_api::configuration_handler::
...
try {
        ...
        } else if (URI_CONFIG_START_CHANNEL_PAIRING ==
                     req_uri.substr(0, URI_CONFIG_START_CHANNEL_PAIRING.size())) {
                handle_start_channel_pairing(req, pr);
        ...
        } else if (URI_CONFIG_CONNECT_SERVER_PUBLIC ==
                     req_uri.substr(0, URI_CONFIG_CONNECT_SERVER_PUBLIC.size())) {
                handle_request_public_key(req, pr);
        } else if (URI_CONFIG_CONNECT_SERVER ==
                     req_uri.substr(0, URI_CONFIG_CONNECT_SERVER.size())) {
                handle_request_keys(req, pr);
        } else if (URI_CONFIG_OPEN_CHANNEL ==
                     req_uri.substr(0, URI_CONFIG_OPEN_CHANNEL.size())) {
                handle_open_channel(req, pr);
...
void leviathan::metax_web_api::configuration_handler::
handle_start_channel_pairing(Poco::Net::HTTPServerRequest& req,
                std::promise<web_api_adapter::ims_response>& pr)
{
        if (! req.clientAddress().host().isLoopback()) {
                throw Poco::Exception("start_channel_pairing request only allowed from"
                                " localhost machine.");
        }
        Poco::Net::HTMLForm f(req, req.stream());
        ...
        if (f.has("key")) {
                ...
                throw Poco::Exception("Key is not specified");
        }
        web_api_adapter* ims = web_api_adapter::get_instance();
        poco_assert(nullptr != ims);
        ims->handle_start_channel_pairing(&pr, timeout, user);
}
...
void leviathan::metax_web_api::configuration_handler::
handle_request_public_key(Poco::Net::HTTPServerRequest& req,
                std::promise<web_api_adapter::ims_response>& pr)
{
        if (! req.clientAddress().host().isLoopback()) {
        ...
        if (f.empty() || ! f.has("ip") || ! f.has("code")) {
                throw Poco::Exception("ip and code parameters need to be specified");
        }
        ...
        web_api_adapter* ims = web_api_adapter::get_instance();
        poco_assert(nullptr != ims);
        ims->handle_request_public_key(&pr, ip, code);
}

void leviathan::metax_web_api::configuration_handler::
handle_open_channel(Poco::Net::HTTPServerRequest& req,
                std::promise<web_api_adapter::ims_response>& pr)
{
        if (! req.clientAddress().host().isLoopback()) {
                throw Poco::Exception("open_channel request only allowed from"
                                " localhost machine.");
        }
        METAX_NOTICE("Received open channel request");
        Poco::UUID u = Poco::UUID::null();
        Poco::Net::HTMLForm f(req, req.stream());
        if (f.empty() || ! f.has("ip")) {
                throw Poco::Exception("ip for opening channel not specified");
        }
        std::string ip = f.get("ip");
        Poco::Net::IPAddress i;
        if (! Poco::Net::IPAddress::tryParse(ip, i)) {
        ...
        ims->handle_open_channel_request(&pr, user, ip);
}
